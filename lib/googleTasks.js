...
export async function fetchAccessToken(session, cancellable = null) {
    const config = await readConfig(cancellable);
...
export async function fetchTasks(button, { notifyResult = false } = {}) {
    ...
    try {
        ...
        const config = await readConfig(cancellable);
        button._isConfigured = Boolean(config?.client_id && config?.client_secret && config?.refresh_token);
